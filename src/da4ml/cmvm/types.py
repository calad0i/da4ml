...
class Op(NamedTuple):
    ...
    option: int
        Option of the operation (e.g., 0 for addition, 1 for subtraction)
    data: int
        Data to be used in the operation (e.g., shift amount when id1 >= 0)
    qint: QInterval
    ...
    Op will always be used in a list. The rules are as follows:
    If id1 >= 0:
        buf[i] = buf[id0] +/- buf[id1] * 2**data (+ if option == 0, - if option == 1)
    If id1 == -1:
        buf[i] = external_buf[id0] # copy from external buffer, usually input
    If id1 == -2:
        buf[i] = relu(buf[id0]) if not sub else relu(-buf[id0])
    If id1 == -3:
    ...
    def __call__(self, inp: list | np.ndarray | tuple, quantize=False, debug=False):
        ...
        for i, op in enumerate(self.ops):
            if op.id1 == -1:  # copy from external buf
                ...
                buf[i] = _relu(v, _i, _f, inv=op.option, round_mode='TRN')
                # buf[i] = max(0, v)
                continue
